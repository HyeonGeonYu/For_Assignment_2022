...
class communicationsystem:
    def __init__(self, ext,inp_data,mapped_data,inp_data_unique_arr, inp_data_unique_arr_idx_arr,count,
        ...
        self.SNR = SNR                                                      # SNR dB
        self.rootpower_of_symbol = cp.sqrt((10**(SNR/10))/(Tx))             # SNR로 계산한 symbol 한개 power의 root
        self.N0 = 1                                                         # N0
...
def modulation(inp_class):
    if inp_class.modulation_scheme == "QPSK":
        ...
        inp_class.modulation_result = cp.zeros(refer_arr.shape[0],dtype='complex')

        inp_class.modulation_result = cp.where((refer_arr == cp.array([0, 0])).all(axis=1),inp_class.rootpower_of_symbol*((1+1j)/cp.sqrt(2)),inp_class.modulation_result)
        inp_class.modulation_result = cp.where((refer_arr == cp.array([1, 0])).all(axis=1),inp_class.rootpower_of_symbol*((-1+1j)/cp.sqrt(2)),inp_class.modulation_result)
        inp_class.modulation_result = cp.where((refer_arr == cp.array([0, 1])).all(axis=1),inp_class.rootpower_of_symbol*((1-1j)/cp.sqrt(2)),inp_class.modulation_result)
        inp_class.modulation_result = cp.where((refer_arr == cp.array([1, 1])).all(axis=1),inp_class.rootpower_of_symbol*((-1-1j)/cp.sqrt(2)),inp_class.modulation_result)
    else:
...
def channel_awgn(inp_class):
    ...
    inp_class.channel_result = cp.zeros((Trans_num,inp_class.Rx,1),dtype='complex')
    reshape_mod_result = inp_class.modulation_result.reshape(Trans_num,inp_class.Tx,1)

    if inp_class.modulation_scheme == "QPSK":
            inp_class.channel_H = 1 / cp.sqrt(2) * cp.random.normal(0, 1, (Trans_num*inp_class.Tx*inp_class.Rx, 2))\
                .view(cp.complex).reshape(channel_shape)
            inp_class.noise_N = 1/cp.sqrt(2)*cp.random.normal(inp_class.mu, 1, (Trans_num*inp_class.Rx, 2))\
                .view(cp.complex).reshape(Trans_num,inp_class.Rx,1)
            for i in range(Trans_num):
                inp_class.channel_result[i] = cp.dot(inp_class.channel_H[i],reshape_mod_result[i]) + inp_class.noise_N[i]
def demodulation(inp_class):
    def sym2bit(inp_class,x_hat):
        demodulation_result = cp.zeros_like(inp_class.channel_coding_result_np).reshape(-1, 2)
        real_arr = x_hat.reshape(-1, 1).real
        imag_arr = x_hat.reshape(-1, 1).imag
        demodulation_result[cp.where((real_arr > 0) & (imag_arr > 0))[0]] = cp.array([0, 0])
        demodulation_result[cp.where((real_arr < 0) & (imag_arr > 0))[0]] = cp.array([1, 0])
        demodulation_result[cp.where((real_arr > 0) & (imag_arr < 0))[0]] = cp.array([0, 1])
        demodulation_result[cp.where((real_arr < 0) & (imag_arr < 0))[0]] = cp.array([1, 1])
        demodulation_result = demodulation_result.reshape(inp_class.channel_coding_result_np.shape)
        return demodulation_result

    if inp_class.modulation_scheme == "QPSK":
        QPSK_sym_arr = inp_class.rootpower_of_symbol * cp.array(
            [[(1 + 1j) / cp.sqrt(2)], [(-1 + 1j) / cp.sqrt(2)], [(1 - 1j) / cp.sqrt(2)], [(-1 - 1j) / cp.sqrt(2)]],
            dtype='complex')
        QPSK_sym_perm = cp.array([p for p in product(QPSK_sym_arr, repeat=inp_class.Tx)], dtype='complex')
...
def make_result_class(inp_file_dir,source_coding_type,channel_coding_type,draw_huffmantree,modulation_scheme,fading_scheme,Tx,Rx,mu,SNR):
    ...
    inp_class.channel_coding_result_np = cp.random.randint(0,2,(2,Tx*1000)) # Tx * (전송 횟수)
