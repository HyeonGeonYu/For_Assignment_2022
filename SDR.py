...
def SDR(inp_class,QPSK_sym_arr):
    ...
    for Trans_num_idx in range(Trans_num):
        y_Re_Im = np.vstack((channel_result__for_SD[Trans_num_idx].real,channel_result__for_SD[Trans_num_idx].imag))/np.sqrt(inp_class.rootpower_of_symbol**2/2) # 보낸 sym을 +-1, +-1j 로 normalization
